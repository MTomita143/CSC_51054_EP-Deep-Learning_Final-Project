# src/models/camembert_xgb_meta_umap.py
import os
import numpy as np
import torch
...
class TextMetaTransformer(BaseEstimator, TransformerMixin):
    def __init__(self, embedder=None, scale_embeddings=True, scale_metadata=True, umap_components=128,
                 num_columns=None, bool_columns=None, list_columns=None, 
                 unuseful_columns=None, categorical_column='source'):
    ...
    def fit(self, X, y=None):
        ...
        if self.umap_components is not None:
            self.umap = umap.UMAP(
                n_components=self.umap_components,
                n_neighbors=15,     # start from 15
                min_dist=0.1,
                metric='cosine',
                random_state=42
                ).fit(emb)
...
def build_pipeline(num_columns, bool_columns, list_columns, unuseful_columns, n_estimators=1000, max_depth=9, umap_components=128):
